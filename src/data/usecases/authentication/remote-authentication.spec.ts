...
describe("RemoteAuthentication", () => {
  test("Should call HttpsPostClient with correct URL", async () => {
  ...
  test("Should call HttpsPostClient with correct body", async () => {
    const { sut, httpPostClientSpy } = makeSut();
    const authenticationParams = mockAuthentication();
    await sut.auth(authenticationParams);
    expect(httpPostClientSpy.body).toEqual(authenticationParams); //compara objetos
  });
  ...
  test("Should throw UnexpectedError if HttpsPostClient returns 404", async () => {
    const { sut, httpPostClientSpy } = makeSut();
    httpPostClientSpy.response = {
